...
if [ -e "$GIT_VERSION_FILE" ]; then
	# Skip updating the file if PSPE_GIT_VERSION_NO_UPDATE is 1.
	results=$(grep '^#define PSPE_GIT_VERSION_NO_UPDATE 1' "$GIT_VERSION_FILE")
	if [ "$results" != "" ]; then
		exit 0
	fi
fi
...
if [ "$GIT_VERSION" == "" ]; then
	...
	echo 'const char *PSPE_GIT_VERSION = "unknown";' >> "$GIT_VERSION_FILE"
	exit 0
fi
...
echo "// This is a generated file." > "$GIT_VERSION_FILE"
echo >> "$GIT_VERSION_FILE"
echo 'const char *PSPE_GIT_VERSION = "'"$GIT_VERSION"'";' >> "$GIT_VERSION_FILE"
echo >> "$GIT_VERSION_FILE"
echo "// If you don't want this file to update/recompile, change to 1." >> "$GIT_VERSION_FILE"
echo "#define PSPE_GIT_VERSION_NO_UPDATE 0" >> "$GIT_VERSION_FILE"
